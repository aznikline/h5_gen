...
close all;
folder = '../JPEGImages';
save_root = '../VOC_train1_HDF5';
hr_root = '../HR_train_patches';
lr_root = '../LR_train_patches';
                ...
                if size(image,3)==3
                    ...
                    for x = 1 + margain : stride : hei-size_label+1 - margain
                        for y = 1 + margain :stride : wid-size_label+1 - margain
                            ...
                            imwrite(subim_label, fullfile(hr_root,filepaths(i).name));
                            imwrite(subim_input, fullfile(lr_root,filepaths(i).name));
                            %<0 =0
                            %figure;
                            %imshow(subim_input);
                            %figure;
